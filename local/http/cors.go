...
func corsWrapper(h http.Handler, logger zerolog.Logger) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for _, corsHeader := range corsHeaders {
			...
			logger.Warn().Msgf(`Multiple entries detected for header "%s". Only one should be set: you should enable the CORS handling in the CLI only if you are application does not handle them.`, corsHeader)
		}
	})
}
